...
def train_ordinal_joy():
  joy_train_dataloader_uncased, joy_val_dataloader_uncased, joy_test_dataloader_uncased = get_bert_data_loader('joy')
  joy_uncased_model_1 = BertForUncasedClassification_1()
  #joy_cased_model_1 = BertForCasedClassification_1()

  joy_uncased_model_2 = BertForUncasedClassification_2()
  #joy_cased_model_2 = BertForCasedClassification_2()

  joy_uncased_model_3 = BertForUncasedClassification_3()
  #joy_cased_model_3 = BertForCasedClassification_3()

  ### has to be based on the data distribution for each emotion
  ### has to be based on the data distribution for each emotion
  class_weights1 = torch.tensor([1.96,1], dtype = torch.float)
  class_weights2 = torch.tensor([1,1.29], dtype = torch.float)
  class_weights3 = torch.tensor([1,3.52], dtype = torch.float)

  final_training_stats_au_1 = []
  for lr in [2e-5, 3e-5, 5e-5]:
    joy_uncased_model_1 = BertForUncasedClassification_1()

    uncased_joy_train_1, model1_joyUncased = train(
      class_weights1,  
      1,
      joy_uncased_model_1,
      joy_train_dataloader_uncased,
      joy_test_dataloader_uncased,
      filepath='joy_uncased_ordinal',
      lr=lr
    )

    final_training_stats_au_1.append(uncased_joy_train_1)
  pd.DataFrame(final_training_stats_au_1[0]).to_csv('./data/models/ordinal/joy/lr_2e-5_model1.csv')
  pd.DataFrame(final_training_stats_au_1[1]).to_csv('./data/models/ordinal/joy/lr_3e-5_model1.csv')
  pd.DataFrame(final_training_stats_au_1[2]).to_csv('./data/models/ordinal/joy/lr_5e-5_model1.csv')
 
  final_training_stats_au_2 = []
  for lr in [2e-5, 3e-5, 5e-5]:
    joy_uncased_model_2 = BertForUncasedClassification_2()

    uncased_joy_train_2, joy_uncased_model2 = train(
      class_weights2,  
      2,
      joy_uncased_model_2,
      joy_train_dataloader_uncased,
    ...
    final_training_stats_au_2.append(uncased_joy_train_2)

  pd.DataFrame(final_training_stats_au_2[0]).to_csv('./data/models/ordinal/joy/lr_2e-5_model2.csv')
  pd.DataFrame(final_training_stats_au_2[1]).to_csv('./data/models/ordinal/joy/lr_3e-5_model2.csv')
  pd.DataFrame(final_training_stats_au_2[2]).to_csv('./data/models/ordinal/joy/lr_5e-5_model2.csv')
  ...
  for lr in [2e-5, 3e-5, 5e-5]:
    ...
    uncased_joy_train_3, joy_uncased_model3 = train(
      class_weights3,  
      3,
      joy_uncased_model_3,
      joy_train_dataloader_uncased,
    ...
    final_training_stats_au_3.append(uncased_joy_train_3)

  pd.DataFrame(final_training_stats_au_3[0]).to_csv('./data/models/ordinal/joy/lr_2e-5_model3.csv')
  pd.DataFrame(final_training_stats_au_3[1]).to_csv('./data/models/ordinal/joy/lr_3e-5_model3.csv')
  pd.DataFrame(final_training_stats_au_3[2]).to_csv('./data/models/ordinal/joy/lr_5e-5_model3.csv')
